...
To test the \emph{death frequency} of a single spin cell, we run a lattice with a single spin for 250MCS. In 250 simulations with the parameters $a_{target}(\sigma(1))=20$ and $J(1,1)=1, J(1,0)=5$ we observe 48 death events ($19.2\%$). Final surviving cells had a mean area of $\mu = 17.99$ with a variance of $\sigma^2 6.39$ (Figure \ref{cellareas}). %TODO%
